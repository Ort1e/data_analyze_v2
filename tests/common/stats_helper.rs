...
use plot_helper::stat::calculate_q1_q3;
use plot_helper::stat::stats_serie::{MetricName, StatsSerie};
...
pub fn test_stats(instance : &TestSerie, verbose : bool) {
    ...
    let (q1, q3) = calculate_q1_q3(&mut instance.data.iter().map(|x| (0.0, *x)).collect::<Vec<(f32, f32)>>());
